...
4. `js/reference-audio.js` (`ReferenceAudio`) — the "▶ Play" button's
...
5. `js/app.js` — orchestrator with no exported global; owns DOM/state,
   wires `data/words.json` + the four modules above into the word-card UI
   (play reference → record → detect pitch → segment into moras → diagram
   + score the learner's attempt against the target), and drives
